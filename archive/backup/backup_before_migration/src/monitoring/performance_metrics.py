...
class MetricsCollector:
    ...
    def _update_aggregations(self, metric_name: str, value: float):
        ...
        if metric_type == MetricType.COUNTER:
            ...
            self.aggregations[metric_name]["min"] = min(
                value, self.aggregations[metric_name].get("min", float("inf"))
            )
            self.aggregations[metric_name]["max"] = max(
                value, self.aggregations[metric_name].get("max", float("-inf"))
            )
    ...
    def get_rate(
        self, metric_name: str, time_window: timedelta = timedelta(minutes=1)
    ) -> float:
...
class CellMetrics:
    ...
    def record_operation(self, operation_name: str, duration_ms: float, success: bool):
        ...
        self.collector.record(
            "operation_latency", duration_ms, {"operation": operation_name}
        )

        if success:
            self.collector.record(
                "operations_completed", 1, {"operation": operation_name}
            )
        else:
            self.collector.record("error_count", 1, {"operation": operation_name})
...
class TissueMetrics:
    ...
    def record_communication(self, from_cell: str, to_cell: str, latency_ms: float):
        ...
        self.collector.record(
            "inter_cell_latency", latency_ms, {"from": from_cell, "to": to_cell}
        )
    ...
    def get_tissue_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quarantine_count": self.collector.aggregations.get(
                "quarantined_cells", {}
            ).get("current", 0),
            "communication_latency": self.collector.get_statistics(
                "inter_cell_latency"
            ),
            "transaction_success": self.collector.aggregations.get(
...
class MetricsDashboard:
    ...
    def get_dashboard_json(self) -> str:
        ...
        dashboard_data = {"timestamp": datetime.now().isoformat(), "tissues": {}}
    ...
    def print_dashboard(self):
        ...
        print("\n" + "=" * 60)
        print(
            f"🏥 CODE ORGANISM HEALTH DASHBOARD - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        print("=" * 60)
        ...
        for tissue_name, metrics in self.tissue_metrics.items():
            ...
            print(
                f"   Cells: {data['cell_count']} active, {data['quarantine_count']} quarantined"
            )
            ...
            if comm_stats:
                print(
                    f"   Inter-cell Latency: {comm_stats.get('mean', 0):.2f}ms (p95: {comm_stats.get('p95', 0):.2f}ms)"
                )
