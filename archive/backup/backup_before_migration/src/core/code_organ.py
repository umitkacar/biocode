...
class CodeOrgan:
    ...
    def hot_swap_tissue(self, old_tissue_name: str, new_tissue: AdvancedCodeTissue) -> bool:
        """Hot swap a tissue without stopping organ function"""
        if old_tissue_name not in self.tissues:
            return False
        
        # Get the role of old tissue
        role = self.tissue_roles.get(old_tissue_name, "general")
        
        # Remove old tissue
        self.remove_tissue(old_tissue_name)
        
        # Add new tissue with same role
        self.add_tissue(new_tissue, role)
        
        # Record transplant
        self.transplant_history.append({
            "timestamp": datetime.now(),
            "old_tissue": old_tissue_name,
            "new_tissue": new_tissue.name,
            "role": role,
        })
    ...
    def get_diagnostics(self) -> Dict[str, Any]:
        ...
        # Add tissue diagnostics
        for name, tissue in self.tissues.items():
            diagnostics["tissues"][name] = {
                "health": tissue.calculate_tissue_health(),
                "cell_count": len(tissue.cells),
                "role": self.tissue_roles.get(name, "unknown"),
            }
