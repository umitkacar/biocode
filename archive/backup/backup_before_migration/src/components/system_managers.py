...
class SystemBootManager:
        ...
        try:
            for stage in self.boot_stages:
                ...
                self.boot_log.append(
                    {"stage": stage_name, "start": datetime.now(), "status": "starting"}
                )
            ...
            logging.info(
                f"System {self.system_name} booted in {self.boot_time.total_seconds():.2f}s"
            )
            return True
    ...
    async def _start_services(self, system):
        ...
        system.maintenance_task = asyncio.create_task(system._maintenance_loop())

        # Start dream state task
        system.dream_state_task = asyncio.create_task(system._dream_state_loop())
...
class MaintenanceManager:
    ...
    async def run_maintenance(
        self, system, tasks: Optional[List[str]] = None
    ) -> Dict[str, Any]:
    ...
    async def _memory_cleanup(self, system) -> Dict[str, int]:
        ...
        recent_items = [
            ...
            and (cutoff_time - item["timestamp"]).seconds < 3600
        ]
        ...
        for (organ1, organ2), strength in system.organ_connections.items():
            if strength < 0.3:
                unhealthy_connections.append(
                    {"organs": (organ1, organ2), "strength": strength}
                )
...
class SystemMemoryManager:
    ...
    async def consolidate_memory(
        self, system, strategy: str = "frequency"
    ) -> Dict[str, Any]:
    ...
    async def _consolidate_by_importance(self, system) -> Dict[str, Any]:
        ...
        important_items = [
            ...
            for item in system.memory.short_term
            if isinstance(item, dict)
            and item.get("duration", 0) > 1.0  # Slow operations
        ]
    ...
    async def _consolidate_by_age(self, system) -> Dict[str, Any]:
        ...
        system.memory.short_term = deque(
            [
                ...
                if isinstance(item, dict)
                and "timestamp" in item
                and (cutoff - item["timestamp"]).seconds < 7200
            ],  # 2 hours
            maxlen=system.memory.short_term.maxlen,
        )

        return {"removed": old_count - len(system.memory.short_term), "strategy": "age"}
...
class SystemShutdownManager:
    ...
    async def shutdown_system(self, system) -> bool:
        ...
        try:
            for stage in self.shutdown_stages:
                stage_name = stage.__name__
                self.shutdown_log.append({"stage": stage_name, "start": datetime.now()})
    ...
    async def _notify_organs(self, system):
        ...
        shutdown_event = {"type": "system_shutdown", "timestamp": datetime.now()}
