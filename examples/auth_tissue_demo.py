...
class TokenCell(CodeCell):
    ...
    def __init__(self, name: str, **kwargs):
        super().__init__(name)
        self.secret_key = kwargs.get('secret_key', 'default-secret')
...
async def authentication_tissue_demo():
    ...
    auth_tissue.inject_dependency('secret_key', 'super-secret-key-123')
    ...
    # 1. Login attempt
    login_result = await login_cell.authenticate("admin", "secret")
