...
class TestCodeOrgan:
    ...
    def test_organ_creation(self):
        ...
        organ = CodeOrgan("liver", CompatibilityType.O)
        
        assert organ.name == "liver"
        assert organ.compatibility_type == CompatibilityType.O
    ...
    def test_compatibility_types(self):
        ...
        o_organ = CodeOrgan("kidney_o", CompatibilityType.O)
        assert o_organ.is_compatible_with(CompatibilityType.A)
        assert o_organ.is_compatible_with(CompatibilityType.B)
        assert o_organ.is_compatible_with(CompatibilityType.AB)
        assert o_organ.is_compatible_with(CompatibilityType.O)
        
        # Type A can donate to A and AB
        a_organ = CodeOrgan("kidney_a", CompatibilityType.A)
        assert a_organ.is_compatible_with(CompatibilityType.A)
        assert not a_organ.is_compatible_with(CompatibilityType.B)
        assert a_organ.is_compatible_with(CompatibilityType.AB)
        assert not a_organ.is_compatible_with(CompatibilityType.O)
    ...
    def test_transplant_preparation(self):
        ...
        organ = CodeOrgan("heart", CompatibilityType.A)
        ...
        assert transplant_data["compatibility_type"] == CompatibilityType.A
        assert transplant_data["health_status"] is not None
        assert "tissues" in transplant_data

    @pytest.mark.asyncio
    async def test_hot_swap_tissue(self):
        ...
        # Hot swap
        success = await organ.hot_swap_tissue("old_hepatic", new_tissue)
    ...
    def test_get_diagnostics(self):
        ...
        assert diagnostics["name"] == "brain"
        assert diagnostics["tissue_count"] == 2
